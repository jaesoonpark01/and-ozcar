        ...
        if (type === 'LOAN_APPLICATION') {
            ...
            if (amount > 5000000) {
                const alert: SecurityAlert = {
                    ...
                    severity: 'HIGH',
                    target: vin,
                    description: `Large loan application detected ($${amount / 100}). Automated secondary verification triggered.`,
                    timestamp: Date.now(),
                    status: 'PENDING'
                };
        ...
        if (type === 'MAINTENANCE_RECORD' && historicalData) {
            ...
            if (mileage < prevMileage) {
                const alert: SecurityAlert = {
                    ...
                    severity: 'CRITICAL',
                    target: vin,
                    description: `CRITICAL: Mileage rollback detected. Prev: ${prevMileage}, New: ${mileage}. AUTO-FREEZE TRIGGERED.`,
                    timestamp: Date.now(),
                    status: 'AUTO_FROZEN'
                };
                this.alerts.push(alert);
            ...
            if (zScore > 3) {
                const alert: SecurityAlert = {
                    ...
                    severity: 'MEDIUM',
                    target: vin,
                    description: `Statistical Anomaly: Mileage increase is outside normal variance (Z-score: ${zScore.toFixed(2)}). Investigation recommended.`,
                    timestamp: Date.now(),
                    status: 'PENDING'
                };
                this.alerts.push(alert);
