...
export default function SentinelDashboard() {
    ...
    // ?ÑÎùº?¥Ïã± ?îÏßÑ ?∞Îèô (?¥Îùº?¥Ïñ∏???¨Ïù¥??Î™®Ïùò ?ôÏûë)
    ...
    // ?¥ÎßÅ(Polling) Î£®ÌîÑ + AI Î∂ÑÏÑù Î°úÏßÅ ?µÌï©
    useEffect(() => {
    ...
    // ?§ÏãúÍ∞??∞Ïù¥??Î≥Ä??Í∞êÏ? Î∞?AI Î∂ÑÏÑù ?îÏßÑ ?∏Ï∂ú
    useEffect(() => {
        if (carData?.voltage) {
    ...
    return (
        <div className="min-h-screen bg-[#07080a] text-slate-100 p-6 md:p-12 font-sans selection:bg-blue-500/30">
            ...
            {/* Trip Summary ?§Î≤Ñ?àÏù¥ (Ï°∞Í±¥Î∂Ä Î™®Îã¨) */}
            {showTripSummary && (
                <TripSummary
                    score={91} // Mock Data
            ...
            {/* HUD Î™®Îìú ?§Î≤Ñ?àÏù¥ (Ï°∞Í±¥Î∂Ä ?åÎçîÎß? */}
            {showHUD && (
                <HUDMode
