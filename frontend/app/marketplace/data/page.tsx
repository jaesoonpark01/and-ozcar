    ...
    const handleCheckout = async (planType: string) => {
        try {
            ...
            if (data.url) {
                ...
                alert("Í≤∞Ï†ú Ï¥àÍ∏∞?îÏóê ?§Ìå®?àÏäµ?àÎã§.");
                setIsLoading(null);
            }
        } catch (error) {
            ...
            <div className="max-w-6xl mx-auto space-y-12">
                ...
                <div className="text-center max-w-3xl mx-auto">
                    ...
                    <p className="text-slate-400">
                        ?§Ï¶àÏπ?Ozcar) Ï§ëÍ≥†Ï∞?ÎßàÏºì?åÎ†à?¥Ïä§??'Î¨¥Í≤∞???∞Ïù¥??Î•?Í∞Ä??Îπ†Î•¥Í≥??àÏ†Ñ?òÍ≤å ?ëÍ∑º?òÏÑ∏?? ?∞Ïù¥?∞Ïùò ?†Î¢∞?ÑÏ? ?¨ÏÜå?±Ïóê Í∏∞Î∞ò??Íµ¨ÎèÖ??API ?úÎπÑ?§Î? ?úÍ≥µ?©Îãà??
                    </p>
                </div>
                    ...
                    >
                        ...
                        <div className="flex flex-col p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-blue-500/30 transition-colors">
                            <div className="mb-6">
                                ...
                                <p className="text-sm text-slate-400">Í∞úÏù∏ Íµ¨Îß§?êÏö© ?®Í±¥ Ï°∞Ìöå</p>
                            </div>
                            <div className="mb-6">
                                <span className="text-4xl font-black italic">$10</span>
                                <span className="text-slate-500 text-sm"> / Í±?/span>
                            </div>
                            <ul className="space-y-3 mb-8 flex-1">
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ?πÏ†ï Ï∞®Îüâ(VIN) ?ïÎ? Î≥µÏõê Í∏∞Î°ù</li>
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ?¨Í≥† ?ÅÏÑ∏ ?¥Ïó≠ Î¶¨Ìè¨??/li>
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> NXP S32K3 Î¨¥Í≤∞??Í≤ÄÏ¶??¨Ìï®</li>
                            </ul>
                            <button 
                        ...
                        <div className="flex flex-col p-8 rounded-3xl bg-gradient-to-b from-blue-900/40 to-black border border-blue-500/50 relative shadow-[0_0_40px_rgba(59,130,246,0.15)] transform md:-translate-y-4">
                            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 bg-blue-600 text-white text-xs font-bold rounded-full">
                                MOST POPULAR (?úÎü¨ ?ÑÏö©)
                            </div>
                            <div className="mb-6 mt-2">
                                <h3 className="text-xl font-bold text-blue-400 mb-2">B2B Starter</h3>
                                <p className="text-sm text-blue-200/60">Ï§ëÏÜå???úÎü¨??Î∞??åÌÑ∞Ïπ??ÖÏ≤¥</p>
                            </div>
                            <div className="mb-6">
                            ...
                            <ul className="space-y-3 mb-8 flex-1">
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ??50??API ÏøºÎ¶¨ ?úÍ≥µ</li>
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ?ÑÏö© API Î∞úÍ∏â Î∞?Webhook ÏßÄ??/li>
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ?îÏó¨ ÏøºÎ¶¨ ?µÏõî ?¥Ïõî Î∂àÍ?</li>
                            </ul>
                            <button 
                        ...
                        <div className="flex flex-col p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-blue-500/30 transition-colors">
                            <div className="mb-6">
                                ...
                                <p className="text-sm text-slate-400">Í∏àÏúµ???ÄÏ∂??¨ÏÇ¨), Î≥¥Ìóò??Î≥¥ÌóòÎ£??∞Ï†ï)</p>
                            </div>
                            <div className="mb-6">
                                <span className="text-4xl font-black italic">Custom</span>
                            </div>
                            <ul className="space-y-3 mb-8 flex-1">
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> Î¨¥Ï†ú??API ÏøºÎ¶¨ (SLA 99.9%)</li>
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ?ÑÎ¶¨ÎØ∏ÏóÑ ÏøºÎ¶¨ (SOH/Í∑πÌïú ?òÍ≤Ω) ?ëÍ∑º</li>
                                <li className="flex gap-3 text-sm text-slate-300"><CheckCircle2 size={18} className="text-blue-400 shrink-0" /> ?ÑÎã¥ Í∏∞Ïà† ÏßÄ??Î∞?ÎßûÏ∂§???∏ÌîÑ??/li>
                            </ul>
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-sm text-slate-400">?ÑÏû¨ Íµ¨ÎèÖ ?åÎûú</div>
                            ...
                            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between text-sm mb-1">
                                        <span className="text-slate-400">?¥Î≤à ??API ?¨Ïö©??/span>
                            ...
                            <div className="p-6 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-sm text-slate-400">API ?ÅÌÉú</div>
